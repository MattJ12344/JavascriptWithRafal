...
do {
	...
	// 4. przelacz gracza na nastapnego 
	...
	let x: number = koordynatyRuchow[numerTury][0];
	console.log("x=" + x);
	let y: number = koordynatyRuchow[numerTury][1];
	console.log("y=" + y);

	if (numerTury % 2 === 0) {
		console.log("Gracz " + secondPlayer + " zaznacza pole:");
		tablica[x][y] = secondPlayer;
	} else if (numerTury % 2 !== 0) {
...
} while (wygranaO === false || wygranaX === false)
...
// 1. Naprawić błąd
// 2. Ma wyświetlić kto wygrał
// 3. Zagrać dla różnych konfiguracji x i y
// 4. Konsolowgowac, gdzie trzeba:
// - wiedzieć jak ma działać program krok po kroku
// 5. Dopisać typy do zmiennych
// 6. Nowy kolcek: sprawdzić czy nie wkładasz x i y na istniejący lub znak