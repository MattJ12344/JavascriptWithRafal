...
class Gracz:
    def __init__(self, nazwa: str, statki: list):
        self.plansza = self.utworz_pusta_plansze(3)
        self.plansza_ataku = self.utworz_pusta_plansze(3)
        self.statki = statki
        self.numer_tury = 0
        self.zajete_pola = []
        self.nazwa = nazwa
    ...
    def czy_mozna_rozmiescic_statek(self, x: int, y: int, rozmiar: int, orientacja: str) -> bool:
        if orientacja == 'poziomo' and y + rozmiar > 3:
            return False
        if orientacja == 'pionowo' and x + rozmiar > 3:
            return False
...
class Statki:
    ...
    def wybierz_statek_i_umiesc(self, gracz) -> None:
        for _ in range(2): 
            while True:
                ...
                x = int(input("Podaj pozycję x (0-2): "))
                y = int(input("Podaj pozycję y (0-2): "))
    ...
    def graj(self) -> None:
        ...
        while True:
            ...
            x = int(input(f"{self.aktualny_gracz.nazwa}, podaj pozycję x do ataku (0-2): "))
            y = int(input(f"{self.aktualny_gracz.nazwa}, podaj pozycję y do ataku (0-2): "))
