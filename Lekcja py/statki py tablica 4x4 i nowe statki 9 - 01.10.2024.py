...
class Gracz:
    def __init__(self, nazwa: str, statki: list):
        self.plansza = self.utworz_pusta_plansze(4)
        self.plansza_ataku = self.utworz_pusta_plansze(4)
        self.statki = statki
        self.numer_tury = 0
        self.zajete_pola = []
        self.nazwa = nazwa
    ...
    def czy_mozna_rozmiescic_statek(self, x: int, y: int, rozmiar: int, orientacja: str) -> bool:
        if orientacja == 'poziomo' and y + rozmiar > 4:
            return False
        if orientacja == 'pionowo' and x + rozmiar > 4:
            return False
...
class Statki:
    ...
    for rozmiar in statki_do_rozmieszczenia:
            while True:
                print(f"{gracz.nazwa}, wybierz pozycję dla statku:")
                rozmiar = int(input("Podaj rozmiar statku (1-3): "))
                x = int(input("Podaj pozycję x (0-3): "))
                y = int(input("Podaj pozycję y (0-3): "))
    ...
    def graj(self) -> None:
        ...
        while True:
            ...
            x = int(input(f"{self.aktualny_gracz.nazwa}, podaj pozycję x do ataku (0-3): "))
            y = int(input(f"{self.aktualny_gracz.nazwa}, podaj pozycję y do ataku (0-3): "))
